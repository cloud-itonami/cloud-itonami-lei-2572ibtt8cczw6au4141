...
(defn- good-candidate [db]
  (let [bl (store/baseline db)
        added "P&G will notify registered users by e-mail at least 30 days before any material change to these Terms takes effect."
...
(defn- exec [actor tid candidate & [extra]]
  (g/run* actor
          {:request (merge {:op :tos/change-proposal :subject "procter-gamble-terms-1"
                            :candidate candidate}
                           extra)
