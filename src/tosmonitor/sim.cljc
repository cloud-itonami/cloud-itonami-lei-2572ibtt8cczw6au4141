...
(defn- run-check!
  ...
  (let [request (merge {:op :tos/change-proposal :subject "procter-gamble-terms-1"
                        :candidate candidate}
                       extra)
...
(defn -main [& _]
  (let [db    (store/seed-db)
        ...
        good-added-line "P&G will notify registered users by e-mail at least 30 days before any material change to these Terms takes effect."
    ...
    (let [db2 (store/seed-db)
          a2  (op/build db2)
          r   (g/run* a2 {:request {:op :tos/change-proposal :subject "procter-gamble-terms-1"
