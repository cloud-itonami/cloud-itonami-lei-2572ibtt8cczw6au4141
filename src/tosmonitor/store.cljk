(ns tosmonitor.store
  ...
  This repo archives exactly ONE company (The Procter & Gamble Company, LEI
  2572IBTT8CCZW6AU4141) -- unlike `commitledger.store`'s multi-application
  ...
  data is a swap, not baked into the actor)."
...
;; ----------------------------- demo data -----------------------------
;; A small, representative subset of the real archive
;; (80-data/public/tos.journal.edn) -- the actual document text there is
;; page-length; this Store, like every sibling actor's `demo-data`, holds a
;; short, self-contained fixture, not the full real archive.
...
(def ^:private demo-full-text
  "Terms of Use for Procter & Gamble Web Sites\nLast Updated: September, 2024\nThese Terms govern your use of P&G Sites. By using the Sites you accept these Terms. Disputes are governed by the laws of the State of Ohio.")

(defn demo-baseline []
  {:full-text    demo-full-text
   :source-url   "https://termsandconditions.pg.com/en-us/"
   ...
   ;; docstring; :cljs falls back to a fixed placeholder -- that build's own
   ;; sha256-mismatch-violations check is already a no-op there regardless)
